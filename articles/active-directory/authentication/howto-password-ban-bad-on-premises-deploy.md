...
1. Choose one or more servers to host the proxy service for password protection.
   ...
   * You can run the proxy service on a domain controller for testing. But that domain controller then requires internet connectivity, which can be a security concern. We recommend this configuration for testing only. It is generally recommended that the proxy service be placed in the internal network, and not placed in a DMZ due to security concerns.
