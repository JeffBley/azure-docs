...
The order in which you add the conditions are important. Azure AD evaluates the each of the conditions from top to bottom to decide which value to emit in the claim. If a user falls into multiple claims conditions, then last one evaluated will determine the value to emit.
